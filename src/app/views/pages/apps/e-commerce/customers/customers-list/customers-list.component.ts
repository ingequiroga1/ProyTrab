...
export class CustomersListComponent implements OnInit, OnDestroy {
  ...
  deleteCustomer(_item: CustomerModel) {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      ...
      this.store.dispatch(new OneCustomerDeleted({ id: _item.clientId }));
  ...
  deleteCustomers() {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      ...
      for (let i = 0; i < this.selection.selected.length; i++) {
        idsForDeletion.push(this.selection.selected[i].clientId);
      }
...
*/
  editCustomer(customer: CustomerModel) {
    ...
    saveMessageTranslateParam +=
      customer.clientId > 0 ? "UPDATE_MESSAGE" : "ADD_MESSAGE";
    const _saveMessage = this.translate.instant(saveMessageTranslateParam);
    const _messageType =
      customer.clientId > 0 ? MessageType.Update : MessageType.Create;
